...
pthread_mutex_t free_block_lock = PTHREAD_MUTEX_INITIALIZER;
...
int inode_create(inode_type i_type) {
    ...
    case T_SOFTLINK:
        inode_table[inumber].softlink = malloc(MAX_FILE_NAME);
        inode_table[inumber].issymlink=true;
        break;
    }
    
    inode_t * file_nod = inode_get(inumber);
    file_nod->hardlink=1;
    file_nod->issymlink=false;
    pthread_mutex_unlock(&inode_table_lock);
    return inumber;
}
...
inode_t *inode_get_lock(int inumber) {
    pthread_mutex_lock(&inode_table_lock);
    inode_t *t=inode_get( inumber);
    pthread_mutex_unlock(&inode_table_lock);
    return t;
}
/**
...
open_file_entry_t *get_open_file_entry_lock(int fhandle) {
    pthread_mutex_lock(&free_open_file_entries_lock);
    open_file_entry_t *t=get_open_file_entry( fhandle);
    pthread_mutex_unlock(&free_open_file_entries_lock);
    return t;
}
